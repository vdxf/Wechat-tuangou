...
import { reqProductBrandList, reqPostProductBrand, reqDeleteProductBrand } from '@/api/product';
...
const handleRequest = async ([PageIndex, PageSize], query) => {
 const { Data:res } = await reqProductBrandList({PageIndex, PageSize, ...query})
...
const handleDelete = async (e:any) => {
  ...
  .then(async () => {
    await reqDeleteProductBrand({Id: e.Id})
    ElMessage({
      type: 'success',
      message: '删除成功',
    })
...
const handleSubmit = async (query: any, Id:any) => {
  await reqPostProductBrand({...query, Id})
  childRef.value.handleReqTableList([1,10])
  dialogShow.value = false
