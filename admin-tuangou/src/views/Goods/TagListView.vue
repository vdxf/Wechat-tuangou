...
import { reqProductTagList, reqPostProductTag, reqDeleteProductTag } from '@/api/product';
...
const handleRequest = async ([PageIndex, PageSize], query) => {
 const { Data:res } = await reqProductTagList({PageIndex, PageSize, ...query})
...
const handleDelete = async (e:any) => {
  ...
  .then(async () => {
    await reqDeleteProductTag({Id: e.Id})
    ElMessage({
      type: 'success',
      message: '删除成功',
    })
...
const handleSubmit = async (query: any, Id:any) => {
  await reqPostProductTag({...query, Id})
  childRef.value.handleReqTableList([1,10])
  dialogShow.value = false
